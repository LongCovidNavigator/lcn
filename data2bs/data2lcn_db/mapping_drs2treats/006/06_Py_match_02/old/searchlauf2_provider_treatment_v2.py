...
# Nur leichte, kontrollierte Alias-Glättung.
# Kein freies Synonym-Matching, keine aggressive Modellierung.
ALIAS_RULES: List[Tuple[str, str, str]] = [
    # pattern, canonical_name, alias_type
    (r"(?i)^ltt[\s\-]*test$", "LTT-Test", "surface_variant"),
    (r"(?i)^cortisol[\s\-]*tagesprofil$", "Cortisol-Tagesprofil", "surface_variant"),
    (r"(?i)^hashimoto\s*t\.?$", "Hashimoto", "abbreviation"),
    (r"(?i)^vitamin[\s\-]*d$", "Vitamin D", "surface_variant"),
    (r"(?i)^q[\s\-]*10$", "Q10", "surface_variant"),
    (r"(?i)^coenzym[\s\-]*q10$", "Coenzym Q10", "surface_variant"),
    (r"(?i)^leaky[\s\-]*gut(\s*syndrom)?$", "Leaky Gut", "surface_variant"),
    (r"(?i)^histamin[\s\-]*(unverträglichkeit|intoleranz)$", "Histamin-Unverträglichkeit", "surface_variant"),
    (r"(?i)^post[\s\-]*vac$", "PostVac", "surface_variant"),
    (r"(?i)^me[\s\/\-]*cfs$", "ME/CFS", "surface_variant"),
    (r"(?i)^brain[\s\-]*fog$", "Brain Fog", "surface_variant"),
]
...
def canonicalize_treatment(raw: str, normalized: str) -> Tuple[str, str, str]:
    """
    Returns:
      canonical_name, alias_name, alias_type

    alias_name ist nur gesetzt, wenn eine explizite Aliasregel gegriffen hat.
    """
    raw_clean = normalize_space(raw)
    norm_clean = light_normalize(normalized or raw)

    for pattern, canonical, alias_type in ALIAS_RULES:
        if re.fullmatch(pattern, raw_clean) or re.fullmatch(pattern, normalized) or re.fullmatch(pattern, norm_clean):
            return canonical, raw_clean, alias_type

    # leichte Oberflächen-Glättung ohne "echte" Aliasentscheidung
    canonical = raw_clean
...
def load_treatments(path: str, text: str, ranges: Dict[str, Tuple[int, int]]) -> List[Dict[str, object]]:
    ...
    for r in rows:
        ...
        canonical, alias_name, alias_type = canonicalize_treatment(raw, normalized)
...
def row_quality_score(row: Dict[str, str]) -> Tuple[int, int, int]:
    """
    Höher ist besser:
    1) review_mapping=0 besser als 1
    2) evidence_strength high > medium
    3) längerer evidence_excerpt besser
    """
    ...
    return (review_rank, ev_rank, excerpt_len)
...
def mapping_row(
    ...
    if treatment["canonical_name"] != treatment["text_raw"]:
        notes_parts.append(f"canonical_treatment={treatment['canonical_name']}")
    ...
    return {
        ...
        "treatment_name_normalized": light_normalize(str(treatment["canonical_name"])),
        ...
        "review_matching": "0",
        "review_matching_notes": "",
        "alias_name": str(treatment["alias_name"]),
        "alias_type": str(treatment["alias_type"]),
        "sort_order": "",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Suchlauf 2 V2 – Provider↔Treatment in Mapping-Format mit Alias + Dedupe")
