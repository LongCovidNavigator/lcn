...
def title_supports_provider(article_title: str, provider_name: str) -> bool:
    ...
    provider_l = lower_clean(provider_name)

    if provider_l not in title_l:
        return False
...
def find_speaker_anchor(text: str, content_start: int, content_end: int, provider_name: str) -> Optional[Tuple[int, int]]:
    ...
    for m in re.finditer(r".{0,250}", content, flags=re.DOTALL):
        pass

    # Suche satzweise im Content
    for sent in re.finditer(r"[^.!?\n]+[.!?\n]?", content, flags=re.DOTALL):
        sent_text = sent.group(0)
        sent_l = lower_clean(sent_text)
        if provider_l in sent_l and any(re.search(p, sent_l) for p in SPEAKER_TEXT_PATTERNS):
            return (content_start + sent.start(), content_start + sent.end())
...
def paragraph_has_stop(para_text: str, other_provider_names: List[str]) -> bool:
    ...
    for other in other_provider_names:
        if lower_clean(other) and lower_clean(other) in para_l:
            return True
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Suchlauf 2 V4 – Provider↔Treatment mit Speaker Window")
