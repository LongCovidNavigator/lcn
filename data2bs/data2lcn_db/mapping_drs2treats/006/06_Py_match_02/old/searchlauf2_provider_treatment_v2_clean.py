...
def canonicalize_treatment(text: str) -> str:
    """
    Nur generische Oberflächen-Normalisierung.
    Keine harte Aliasliste, keine Synonyme, keine semantische Umdeutung.
    """
    text = normalize_space(text).lower()
    text = text.replace("–", "-").replace("—", "-")
    text = text.replace("/", " ")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Suchlauf 2 V2 clean – Provider↔Treatment mit generischer Normalisierung + Dedupe")
