...
def expand_treatment_phrase(full_text: str, start: int, end: int, raw: str, group: str) -> tuple[int, int, str]:
    ...
    m = re.match(r"^\s+(?:von|mit)\s+([A-ZÄÖÜa-zäöüß0-9\-/]+(?:\s+[A-ZÄÖÜa-zäöüß0-9\-/]+){0,4})", tail)
    if not m:
        return start, end, raw

    phrase_part = m.group(0)
    # harte Stoppwörter gegen zu weites Auslaufen
    stop_word_match = re.search(r"(hörst|liest|findest|kann|können|wird|werden|ist|sind|war|waren|gibt|geben|zeigt|zeigen|hilft|helfen|spricht|sprechen)", phrase_part, flags=re.IGNORECASE)
    if stop_word_match:
