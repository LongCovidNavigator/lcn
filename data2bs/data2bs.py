...
def seite_existiert(title):
    response = requests.get(
        f"{BOOKSTACK_API_URL}/books/{BOOK_ID}/pages",
        headers=auth_headers(),
        params={"count": 1000}
    )
    if response.status_code == 200:
        pages = response.json().get("data", [])
        return any(p["name"].strip().lower() == title.strip().lower() for p in pages)
    return False
